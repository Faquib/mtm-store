...
const NewProduct = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.preference !== undefined && formData.preference !== null && formData.preference !== '') {
      const prefNum = Number(formData.preference);
      if (isNaN(prefNum)) {
        errors.preference = 'Preference must be a number';
      } else if (prefNum < 0) {
        errors.preference = 'Preference must be 0 or higher';
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Update preference separately
      if (formData.preference !== undefined) {
        const prefUpdated = await updateProductPreference();
        if (!prefUpdated) {
          setIsLoading(false);
          return; // Stop if preference update failed
        }
      }
      ...
      // Special handling for preference conflicts
      if (err.response?.data?.conflicting_product) {
        toast.info(
          `Preference conflict with product: ${err.response.data.conflicting_product}`,
          {
            autoClose: 8000,
            position: "top-center"
          }
  ...
  const updateProductPreference = async () => {
    if (formData.preference === undefined) return true; // No preference change

    try {
      const response = await axios.put(
        `${import.meta.env.VITE_SERVER_API}/${editingProduct.product_id}/preference`,
      ...
      if (err.response?.data?.error) {
        ...
        if (err.response.data.conflicting_product) {
          toast.info(
            `Conflict with product: ${err.response.data.conflicting_product} (ID: ${err.response.data.conflicting_product_id})`,
            {
              autoClose: 5000
            }
          );
  ...
  const renderProductForm = () => (
    <div className="product-form-container">
      ...
      <form onSubmit={handleSubmit} className="product-form" ref={formRef}>
        ...
        <div className={`form-group ${validationErrors.preference ? 'error-field' : ''}`}>
          ...
          <input
            ...
            onChange={handleInputChange}
            placeholder="Enter unique preference number (0 or higher)"
            min="0"
            step="1"
          />
          {validationErrors.preference && (
            <span className="error-message">{validationErrors.preference}</span>
          )}
          <p className="form-note">
            {formData.preference !== undefined && formData.preference !== null && formData.preference !== ''
              ? `Current preference: ${formData.preference}`
              : 'No preference set'}
          </p>
        </div>
