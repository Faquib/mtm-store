...
const OrderDetails = () => {
  ...
  const [fulfillmentMethod, setFulfillmentMethod] = useState('delhivery');
  ...
  useEffect(() => {
    if (showAssignModal && inputRefs.current[0]) {
      inputRefs.current[0].focus(); // Auto-focus first input when modal opens
    }
  }, [showAssignModal]);
      ...
      <div className="order-details-container">
        ...
        {areAllSrNosAssigned() && (
          <div className="fulfill-order-section">
            ...
            {order.address?.is_available && (
              <div className="fulfillment-method-selection">
                <label>
                  <input
                    type="radio"
                    name="fulfillmentMethod"
                    value="delhivery"
                    checked={fulfillmentMethod === 'delhivery'}
                    onChange={() => setFulfillmentMethod('delhivery')}
                  />
                  Delhivery Service
                </label>
                <label>
            ...
            {/* Show Delhivery button if selected and available */}
            {order.address?.is_available && fulfillmentMethod === 'delhivery' && (
              <>
                <button
                ...
                >
                  {isFulfillingOrder ? 'Processing...' : 'Fulfill with Delhivery'}
                </button>
            ...
            {/* Show manual buttons if Delhivery not available OR manual selected */}
            {(order.address?.is_available === false || fulfillmentMethod === 'manual') && (
        ...
        <div className="sr-assign-modal">
          <div className="sr-assign-modal-content">
            ...
            <div className="sr-assign-modal-body">
                      ...
                      <td>
                        <input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              e.preventDefault();
                              // Jump to next input if available
                              if (index < currentProduct.details.length - 1 && inputRefs.current[index + 1]) {
              ...
              {/* New Bulk Input Section */}
              <div className="bulk-sr-input-section">
                <h4>Bulk Assign SR Numbers</h4>
                <textarea
                  className="bulk-sr-textarea"
                  placeholder={`Enter all ${currentProduct.details.length} SR numbers separated by commas (e.g., "SR001,SR002,SR003")`}
                  onChange={(e) => {
                    ...
                    // Update only if we have correct number of values
                    if (values.length === currentProduct.details.length) {
                      const newInputs = { ...srNoInputs };
                      currentProduct.details.forEach((detail, idx) => {
                        newInputs[detail.detail_id] = values[idx];
                      });
                      setSrNoInputs(newInputs);
                    }
                  }}
                />
                <div className="bulk-input-hint">
                  <p>Enter exactly {currentProduct.details.length} SR numbers separated by commas</p>
                  <p>Example: SR001,SR002,SR003,...,SR{currentProduct.details.length}</p>
                </div>
              </div>
            </div>
