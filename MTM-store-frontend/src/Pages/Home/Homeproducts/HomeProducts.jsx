...
const HomeProducts = () => {
  ...
  // Update the product sorting logic
  useEffect(() => {
    if (products.length > 0) {
      // Sort by preference (lower numbers first), then take first 12
      const sortedByPreference = [...products].sort((a, b) => {
        const aPref = a.preference || Number.MAX_SAFE_INTEGER;
        const bPref = b.preference || Number.MAX_SAFE_INTEGER;
        return aPref - bPref;
      }).slice(0, 12);
      
      setDisplayedProducts(sortedByPreference);
    }
  }, [products]);
